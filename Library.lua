...
-- VARIANT WARNA NOTIF (Info, Success, Warning, Error)
...
function Library:Notify(...)
    ...
    if typeof(Info) == "table" then
        ...
        Data.Type = Info.Type or Info.Variant
    ...
    -- Terapkan variant warna
    local RawVariant = tostring(Data.Type or ""):lower()
    ...
    -- BIG ICON di kiri (kayak Olympus)
    local IconLabel
    ...
    local VariantLabel = New("TextLabel", {
        ...
        TextTruncate = Enum.TextTruncate.AtEnd,
        TextXAlignment = Enum.TextXAlignment.Left,
        Visible = Data.Type ~= nil and Data.Type ~= "",
        ZIndex = HeaderText.ZIndex + 1,
        Parent = HeaderText,
    })
...
-- HELPER FUNCTIONS (biar gampang pakenya)
function Library:NotifyInfo(Text, Time)
    if typeof(Text) == "table" then
        Text.Type = Text.Type or "Info"
        return Library:Notify(Text)
    end
...
function Library:NotifySuccess(Text, Time)
    if typeof(Text) == "table" then
        Text.Type = Text.Type or "Success"
        return Library:Notify(Text)
    end
    return Library:Notify({
        Type = "Success",
...
function Library:NotifyWarning(Text, Time)
    if typeof(Text) == "table" then
        Text.Type = Text.Type or "Warning"
        return Library:Notify(Text)
    end
    return Library:Notify({
        Type = "Warning",
...
function Library:CreateWindow(WindowInfo)
    ...
    do
        ...
        -- OUTLINE UNGU WINDOW (2 lapis kayak Olympus)
local OutlineStroke = New("UIStroke", {
    Color = "AccentColor",
    Thickness = 1.5,
    Transparency = 0.08,
