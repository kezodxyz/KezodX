...
function Library:AddDraggableButtonv2(Text: string, Func, ExcludeScaling: boolean?)
    ...
    function Table:SetText(NewText: string)
        if IsIcon then
            ...
            task.spawn(function()
                task.wait(0.05)
                local IconImg = Button:FindFirstChildOfClass("ImageLabel")
                if IconImg then
                    -- Border ungu berputar
                    ...
                    Gradient.Color = ColorSequence.new({
                        ...
                        ColorSequenceKeypoint.new(0.35, Color3.fromRGB(200, 120, 255)),
                        ColorSequenceKeypoint.new(0.5, Color3.fromRGB(150, 70, 255)),
                        ColorSequenceKeypoint.new(0.65, Color3.fromRGB(100, 40, 180)),
                    ...
                    Glow.Color = Color3.fromRGB(140, 60, 220)
                    Glow.Thickness = 7
                    ...
                    RunService.RenderStepped:Connect(function(dt)
                        if not IconImg.Parent then return end
                        Gradient.Rotation = (Gradient.Rotation + 180 * dt) % 360
                        pulseTime = pulseTime + dt
                        local t = (math.sin(pulseTime * 2.2) + 1) / 2
                        Glow.Transparency = 0.55 + (0.28 * (1 - t))
                        Glow.Thickness = 6 + (3 * t)
                    end)
                end
            end)
        else
